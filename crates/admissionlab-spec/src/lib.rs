...
//!
//! # Two wire versions, one resolved model
//!
//! ROADMAP Task 7.1 promotes the lab document to
//! `admissionlab.io/v1beta1` while keeping `admissionlab.io/v1alpha1`
//! readable through at least v1.0. The whole of that lives in three
//! modules and one function:
//!
//! - [`v1alpha1`] is the **frozen** Public Alpha wire model
//!   ([`V1Alpha1Lab`]). It never changes shape again.
//! - [`v1beta1`] is the current wire model ([`V1Beta1Lab`]).
//! - [`model`] holds every hand-written type both versions spell
//!   *identically* — which is most of them; a type lives there exactly
//!   while no supported version disagrees about it.
//! - [`migrate_v1alpha1_to_v1beta1`] is the one bridge, and it is pure.
//!
...
//! carries no version marker at all: every document is migrated to the
//! current model *before* resolution, so there is exactly one resolver
//! and one resolved shape for the rest of the workspace to consume. See
//! [`ResolvedLab`]'s "Version-independent by construction", and
//! `migrate.rs` for the field-by-field record of what the Beta freeze
//! changed (two wire names) and what it deliberately kept.
//!
//! # The pieces
//!
...
//! - [`v1beta1_json_schema`] and [`v1alpha1_json_schema`] generate the
...
//! crate's root are the **current** (`v1beta1`) types — that is what
//! those names have always meant and what [`ResolvedLab`] carries. Their
...
//! name it; the current root is [`V1Beta1Lab`].
...
pub use migrate::{MigrationError, migrate_v1alpha1_to_v1beta1};
pub use model::{
...
pub use schema::{v1alpha1_json_schema, v1beta1_json_schema};
